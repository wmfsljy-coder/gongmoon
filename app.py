...
with col2:
    ...
    if st.button("전문가 정밀 검토 시작", type="primary", use_container_width=True):
        if user_content:
           ...
           **[규정 1. 항목 체계 및 제목]**
            - **항목 1. (근거/관련)**: 반드시 1. 관련: 기관명-번호(연. 월. 일., "제목") 형식을 최상단에 배치하십시오.
            - **항목 2. (본문 시작)**: 핵심 내용을 `2. [주제]을 다음과 같이 건의하고자 합니다.`(또는 보고합니다)로 시작하십시오.
            - '2. 주제을 다음과 같이 보고합니다.' 형식에서 주제어에 **대괄호 [ ]를 절대 사용하지 마십시오.** (예: 2. 연수 실시를 다음과 같이...)
            - 2번 항목 문장이 끝나면 **반드시 줄 바꿈(Enter)**을 한 뒤 다음 줄에 '가.' 항목을 시작하십시오.

            2. **줄 바꿈 및 들여쓰기**:
               - 모든 항목(1., 2., 가., 나., 1), 2))은 반드시 **새로운 줄에서 시작**해야 합니다.
               - 한 줄에 여러 항목을 나열하는 것을 엄격히 금지합니다.
            - '1. 관련:' 문장 바로 다음 줄에 '2.' 항목을 쓰시오. 
