...
**[출력 형식 (매우 중요)]**
결과물을 반드시 아래의 세 가지 구분자로 감싸서 출력하십시오. 다른 안내 멘트는 절대 적지 마십시오.
[제목시작]
(교정된 제목 1줄)
[제목끝]
[본문시작]
...
**[항목 체계 및 강제 줄 바꿈 규정]**
1. **항목 1번과 2번 사이**: 줄 바꿈은 하되, 사이에 빈 줄(Enter 2번)을 절대 두지 마십시오.
2. **항목별 단독 줄 원칙**: '가.', '나.', '다.' 등 모든 하위 항목은 반드시 새로운 줄에서 시작하십시오. (한 줄에 이어서 쓰는 것 금지). 모든 항목 부호 앞에는 줄 바꿈(\n) 필수.
3. **붙임 수직 정렬**: 붙임이 2개 이상일 경우, 1번과 2번 파일명은 반드시 서로 다른 줄에 기재하십시오. 2번 이후의 항목은 반드시 1번 항목의 첫 글자 위치에 수직으로 맞추십시오 (앞에 공백 6칸).
4. **기호 제거**: 2번 항목의 주제어에 대괄호 [ ]를 절대 사용하지 마십시오. (예: 2. 주제를 다음과 같이...)

**[규정 2. 8단계 들여쓰기 (물리적 공백)]**
- 1단계(1., 2.): 0칸 공백
- 2단계(가., 나.): 왼쪽에서 2칸 공백
- 3단계(1), 2)): 왼쪽에서 4칸 공백
- 본문 끝과 '붙임' 사이만 빈 줄 2개를 둡니다.
